  ...
  <Tab title="Vercel AI SDK" icon="/images/vercel.svg">
    <Card>
      ...
      * Created Date (Date)
      ...
      ## EXECUTION POLICY
      - The agent should execute the full workflow automatically without asking for confirmations at each step.
      - Only prompt the user if required data is missing (e.g., no Slack channel ID provided, missing email address, or incomplete lead details).
      - Default to autonomous execution with safe fallbacks and logging for transparency.
      ```
